...
void FEditTableView::onTrim(QString fileName)
{
    ...
    for (int i = 0; i < editItemModel->rowCount(); i++) // for each edit of file
    {
        ...
        int duration = inTime.msecsTo(outTime) + 40;
...
//                    qDebug()<<"trim"<<parent <<command<< result;
    ...
    //                    qDebug()<<"exiftool update date"<<parent <<command<< result;
        ...
        if ( file.open(QIODevice::WriteOnly) )
        {
            ...
            stream << 1 << endl;
            stream << QTime::fromMSecsSinceStartOfDay(deltaIn).toString("HH:mm:ss.zzz") << " --> " << QTime::fromMSecsSinceStartOfDay(duration - deltaOut -  40).toString("HH:mm:ss.zzz") << endl;
